...
public class AStar extends FifteenPuzzleProblem {
	 ...
	 */
	public AStar() {
		performAStar(myCurrentStateString);
	}
	
	/**
	 * Uses the AStar Algorithm to solve the puzle.
	 * 
	 * @param theState
	 */
	@SuppressWarnings("unused")
	private static void performAStar(String theState) {
		int depth = 0;
				...
				if ((child = getUnvisitedChildNode(node, i)) != null) {
					...
					if (pQueue.size() > myMaxFringe) {
						...
						if (myMaxFringe % 5000 == 0 && DEBUG_MODE) {
							...
							System.out.println("next up fringe: " + ((Node)pQueue.peek()).getMyHeuristic());
